...
class Execution
{
    ...
    {
        if(!isset($this->_steps[$nb]))
        {
            throw new ExecutionException('The step number '.$nb.' doesn\'t exist.');
        }
        $jobNameSpace = str_replace('\\\\','\\',AutoCron::JOB_NAMESPACE.$this->_steps[$nb]->getInfo('namespace'));
        $className = $jobNameSpace.$this->_steps[$nb]->getInfo('class');
